...
# =============================================================================
# Posterior path policy
# =============================================================================
# The first version could accidentally match priors/samples paths in GWTC files.
# This policy fixes that by strongly preferring posterior samples and rejecting
# prior/config/calibration/injection/PSD paths by default.
POSTERIOR_PATH_HINTS = [
    "posterior_samples",
    "posterior",
    "samples/posterior",
    "publication",
...
def find_dataset_path(
    ...
    aliases: List[str],
    posterior_only: bool = True,
    allow_prior: bool = False,
) -> Optional[str]:
    """
    Posterior-aware dataset matcher.

    Priority:
      1. reject prior/config/calibration/injection/PSD paths by default;
      2. if posterior_only=True, require posterior-like path hints;
      3. exact basename / suffix match;
      4. contains match only as fallback.

    This is intentionally stricter than the first version because GWTC HDF5 files
    often contain priors/samples/<parameter>, which are not posterior samples.
    """
    ...
    def is_rejected(full: str) -> bool:
        if allow_prior:
            return False
        p = norm(full)
        for bad in REJECT_PATH_HINTS_DEFAULT:
            b = bad.lower()
    ...
    def is_posterior_like(full: str) -> bool:
        p = full.lower().replace("\\", "/")
        return any(h in p for h in POSTERIOR_PATH_HINTS)

    # Candidate pool with hard rejection.
    pool = []
    for full, info in datasets.items():
        if is_rejected(full):
            continue
        if posterior_only and not is_posterior_like(full):
            continue
        pool.append(full)

    # If strict posterior-only found no pool, do NOT fall back to priors.
    if posterior_only and not pool:
        return None

    def score_path(full: str, match_rank: int) -> Tuple[int, int, int, int, str]:
        """
        Lower score is better.
        match_rank: exact basename 0, suffix 1, contains 2.
        Prefer posterior-like, 1D arrays, longer sample arrays, shorter path.
        """
        info = datasets[full]
        shape = info.get("shape", ())
        is_1d_penalty = 0 if len(shape) == 1 else 1
        n = int(shape[0]) if len(shape) >= 1 and isinstance(shape[0], (int, np.integer)) else 0
        posterior_penalty = 0 if is_posterior_like(full) else 1
        # negative n because larger arrays are better
        return (match_rank, posterior_penalty, is_1d_penalty, -n, full)

    candidates = []

    # Exact basename match.
    for full in pool:
        base_l = datasets[full]["basename"].lower()
        if base_l in aliases_l:
            candidates.append((score_path(full, 0), full))
    ...
    for full in pool:
        ...
        for a in aliases_l:
            if full_l.endswith("/" + a) or full_l == a:
                candidates.append((score_path(full, 1), full))
    ...
    for full in pool:
        ...
        for a in aliases_l:
            if a in base_l or a in full_l:
                candidates.append((score_path(full, 2), full))
...
def extract_event_summary(file_path: str | Path, max_samples: Optional[int] = None, posterior_only: bool = True, allow_prior: bool = False) -> Dict[str, Any]:
    """
    ...
    for canonical, aliases in PARAM_ALIASES.items():
        ds_path = find_dataset_path(datasets, aliases, posterior_only=posterior_only, allow_prior=allow_prior)
        if ds_path is None:
            continue
        try:
...
def audit_extraction_paths(event_df: pd.DataFrame, out_dir: Path, posterior_only: bool = True) -> pd.DataFrame:
    """
    Write a path audit table and flag prior/config contamination.
    """
    rows = []
    path_cols = [c for c in event_df.columns if c.endswith("_path")]
    for _, r in event_df.iterrows():
        ...
        for c in path_cols:
            ...
            if not isinstance(val, str) or not val:
                continue
            p = val.lower().replace("\\", "/")
            is_prior = ("prior" in p or "priors" in p)
            is_posterior = any(h in p for h in POSTERIOR_PATH_HINTS)
            is_rejected = any(h.strip("/").lower() in p for h in REJECT_PATH_HINTS_DEFAULT)
            rows.append({
                "event": event,
                "parameter": c[:-5],
                "path": val,
                "posterior_like": bool(is_posterior),
                "prior_like": bool(is_prior),
                "reject_hint": bool(is_rejected),
                "passes_policy": bool((is_posterior or not posterior_only) and not is_rejected),
            })
    audit = pd.DataFrame(rows)
    if not audit.empty:
        audit.to_csv(out_dir / "gwtc4_extraction_path_audit.csv", index=False)
    return audit
...
def build_event_table(files: List[str], max_samples: Optional[int], out_dir: Path, posterior_only: bool = True, allow_prior: bool = False) -> pd.DataFrame:
    rows = []
    errors = []
    ...
    for i, f in enumerate(files, 1):
        ...
        try:
            row = extract_event_summary(f, max_samples=max_samples, posterior_only=posterior_only, allow_prior=allow_prior)
            rows.append(row)
        except Exception as exc:
...
def main():
    ...
    parser.add_argument("--allow-prior", action="store_true",
                        help="Allow matching prior/prior-sample paths. Default is OFF.")
    parser.add_argument("--no-posterior-only", action="store_true",
                        help="Disable strict posterior-like path requirement. Default requires posterior-like paths.")
    ...
    # Extract event table.
    posterior_only = not args.no_posterior_only
    event_df = build_event_table(
        event_files,
        max_samples=args.max_samples,
        out_dir=out_dir,
        posterior_only=posterior_only,
        allow_prior=args.allow_prior,
    )
    if event_df.empty:
    ...
    path_audit = audit_extraction_paths(event_df, out_dir=out_dir, posterior_only=posterior_only)
    if not path_audit.empty:
        prior_count = int(path_audit["prior_like"].sum())
        bad_count = int((~path_audit["passes_policy"]).sum())
        print(f"[path audit] rows={len(path_audit)} prior_like={prior_count} policy_fail={bad_count}")
        if prior_count > 0 and not args.allow_prior:
            raise RuntimeError(
                "Extraction path audit found prior-like paths while --allow-prior is not set. "
                "Inspect outputs_gwtc4_wct/gwtc4_extraction_path_audit.csv"
            )

    run_summary["n_events_extracted"] = int(len(event_df))
    run_summary["event_columns"] = list(event_df.columns)
    run_summary["posterior_only"] = bool(posterior_only)
    run_summary["allow_prior"] = bool(args.allow_prior)
    run_summary["path_audit_rows"] = int(len(path_audit)) if not path_audit.empty else 0
    run_summary["path_audit_prior_like"] = int(path_audit["prior_like"].sum()) if not path_audit.empty else 0
    run_summary["path_audit_policy_fail"] = int((~path_audit["passes_policy"]).sum()) if not path_audit.empty else 0
    ...
    print(f"[save] {out_dir / 'gwtc4_extraction_path_audit.csv'}")
