...
nonuniformity of scan-max p-values across 168 catalog scans**. A subsequent
arithmetic audit now included in this repository gives:

| aggregate statistic | historical report | literal recomputation | nominal one-sided Gaussian equivalent |
|---|---:|---:|---:|
| p-value distribution nonuniformity | `chi^2 = 95.6`, `p_unif < 1e-15` | `p = 4.12e-16` for `df = 10` | **`Z = 8.05 sigma`** |
| 47 / 168 scans with `p < 0.10` | labeled Poisson-tail `p < 1e-10` | Poisson(`lambda=16.8`): `p = 1.16e-9`; exact Binomial(168, 0.10): `p = 4.85e-11` | **5.97 sigma Poisson / 6.47 sigma exact-binomial** |
| 16 / 168 scans with `p < 0.05` | labeled Poisson-tail `p = 1.4e-3` | Poisson(`lambda=8.4`): `p = 0.0125`; exact Binomial(168, 0.05): `p = 0.0105` | about **2.24-2.31 sigma** |

The historical paper text also says "11 equal bins," while its plotted bins and
reported expected counts correspond to `[0,.05]`, `[.05,.10]`, followed by
nine width-`.10` bins. The audit uses the figure-compatible bin edges because
they reproduce the stated expectations 8.4, 8.4, then 16.8 for `N = 168`.

Run the arithmetic audit directly:

```bash
python scripts/run_gwtc_population_global_null.py --audit-reported
```

These sigma values remain **nominal aggregate p-to-Z conversions**. They are not
single-event significances, do not establish WCT attribution, and should not be
described as a globally calibrated particle-physics-style discovery threshold.
...
`scripts/run_gwtc_population_global_null.py`. It accepts the **observed 168 scan
p-values** and a **null-catalog p-value matrix** generated by the identical
168-scan pipeline. Each row of the null matrix must be one complete synthetic
or null catalog and each column must correspond to the same scan definition and
ordering as the observed vector. This preserves the empirical cross-scan
dependence rather than pretending the 168 p-values are independent.
...
The exact historical 168-value scan vector and its selector manifest are not
currently committed in this repository. Until they are restored or regenerated,
the arithmetic audit is reproducible but the historical aggregate result is not
yet fully reconstructible end-to-end from the repository alone.
